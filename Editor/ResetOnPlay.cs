    ...
    public static class DomainReloadGuard
    {
        ...
        public static void ResetAllStaticState()
        {
            ...
            foreach (var t in classes)
            {
                ...
                {
                    var method = t.GetMethod(a.ResetMethodName, BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy);
                    if (method != null)
                    {
                    ...
                    {
                        Debug.LogWarning($"Couldn't find reset method '{a.ResetMethodName}' for class: {t.Name}. Maybe it's not static?");
                    }
                }
                resetCount++;
            }
